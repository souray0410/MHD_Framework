...
class MHDResNet(MHD_Graph):
    ...
    def __init__(self, config: ResNetConfig, *, weights=None, device='cpu'):
        ...
        # Residual addition belongs to ResidualAddReLU above. Every node has
        # one producer and must discard its previous forward's value.
        nodes = [MHD_Node(i, name, MHD_Node.Message(torch.zeros(1)), aggregation='sum', memory=False)
