...
def test_densenet_complete_native_parity(name):
    ...
    for k,v in model.native_state_dict().items():
        torch.testing.assert_close(v,native.state_dict()[k],rtol=1e-5,atol=1e-6)
    ...
    with torch.no_grad():
        ...
        torch.testing.assert_close(full,model.forward_from('denseblock2',features),rtol=1e-5,atol=1e-6)
