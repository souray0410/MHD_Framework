...
@pytest.mark.skipif(os.environ.get('MHD_EXTENDED_MODELS') != '1', reason='Full extended architecture acceptance is opt-in via MHD_EXTENDED_MODELS=1')
def test_native_graph_outputs_gradients_updates_and_resume(name):
    ...
    for _ in range(2):
        ...
        for key,val in graph.native_state_dict().items():
            try:torch.testing.assert_close(val,native.state_dict()[key],rtol=1e-5,atol=1e-6)
...
def test_convnext_v2_fp32_native_gradients(name):
    ...
    torch.testing.assert_close(x.grad,ref.grad,rtol=1e-5,atol=1e-6)
