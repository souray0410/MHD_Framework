...
Explicit 2D MHD V4 graph using `torchvision_convnext_0_23`. Architecture definitions are independent of datasets, training workflows and initialization checkpoints.
...
The `features` endpoint includes native pooling, channel normalization and flattening; `logits` uses only the final linear layer. The standard optimizer groups the complete native classifier (including this normalization) at the declared head learning rate.
...
[Reference](https://docs.pytorch.org/vision/0.23/models/convnext.html)
