...
| Field | Value |
|---|---|
| Architecture ID | `resnet50_2d_torchvision` |
...
Block: `Bottleneck`; blocks per stage: `[3, 4, 6, 3]`; stage output channels: `[256, 512, 1024, 2048]`. Default graph granularity is `block`; explicit `stage` granularity is also supported.
...
Optional initialization uses an explicit torchvision ImageNet weight release; `weights=None` is random initialization.
...
Native-reference forward/gradient/update checks cover this variant on CPU and single GPU. Complete bundle lifecycle checks additionally use the ResNet18 2D fixture. No trained task weights are registered.
...
Torchvision bottleneck variants use the ResNet V1.5 stride placement. This distinction is part of the implementation identity.
